...
\end{itemize}
\begin{observación} Gracias a la definición anterior podemos ver que: \newline $Y \backsim N(\mu,\sigma^2I_{n})$ ya que:
$$
...
\begin{itemize}
  ...
  \item $c(y,\phi) = \frac{-y^2}{2\phi}-log(\sqrt{\phi 2 \pi}) = \frac{-y^2}{2\sigma} - log(\sigma \sqrt{2\pi})$
\end{itemize}
\end{ejemplo}
...
$$
Y utilizando que: $E[\frac{\partial^2 l}{\partial \theta^2}] = -E[(\frac{\partial l}{\partial \theta})^2]$, nos queda: 
$$
...
$$
Hemos utilizado que $\frac{\partial\mu_i}{\partial \eta_i} = \frac{1}{g'(\mu_i)}$, que $\frac{\partial\eta_i}{\partial \beta_j} = X_{ij}$ y que derivando el resultado $\ref{Esperanza exponencial}$ se tiene: $\frac{d\theta_i}{d \mu_i} = \frac{1}{b''(\theta)}$. Sustituyendo también el resultado de $\ref{V}$: 
\begin{equation}
...
\begin{definición}[Algoritmo de mínimos cuadrados reponderados iterativamente] $\hspace{0,8cm}$
...
$$
Deben tener media nula y varianza $\phi$ si el modelo es correcto.
\end{definición}
