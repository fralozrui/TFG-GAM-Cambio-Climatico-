...
\begin{itemize}
  ...
  \item $\theta$ es el correspondiente vector de parámetros, que antes denotábamos por $\beta$, para las variables predictoras mencionadas en el anterior punto.
...
\label{eq: basic MAG}
\end{equation}
Donde $y_i$ será la variable de respuesta y los $\epsilon_i$ son variables aleatorias independientes e identicamente distribuidas como $N(0,\sigma^2)$ que representan el error.
...
[^2]: Se supone que los nodos están espaciados de manera uniforme, pues en el caso de no que lo estuvieran habría que añadir pesos a la suma.
...
Sea $\hat{f_i}^{[-i]}$ la estimación de la función de suavizado ajustada por todos los datos $\{(x_j,y_j)\}_{j=1}^n$ menos el i-ésimo, se define la validación cruzada ordinaria como:
\begin{equation}
...
$$
f_1(x) = \sum_{j=1}^{k_1} b_j(x)\delta_j \hspace{0.8cm} 
...
[^3]: No nos adentramos en los MLG ya que hace falta su desarrollo para la comprensión del algoritmo.
